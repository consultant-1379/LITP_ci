...
#name1=( $(ssh -p 29418 gerrit.ericsson.se gerrit ls-projects --match ERIClitp | egrep -v "\-testware|ERIClitpvirtualdevenv|ERIClitpnassfs|ERIClitpjavaeeapi") )  
home_dir=/home/xnorhea/projworkspace
name1=( $(cat $home_dir/extr_repo_list ) )
cd $home_dir
#
# remove output file if exists
#
if [ -f "$home_dir/extr_changes.txt" ]; then
	rm $home_dir/extr_changes.txt
fi
#
...
#
echo "Sorry, this output is not very pretty" >> $home_dir/extr_changes.txt
echo "This is the list of commits since the start of the sprint" >> $home_dir/extr_changes.txt
echo "Note that ERIClitpdocs is OMITTED" >> $home_dir/extr_changes.txt
echo "" >> $home_dir/extr_changes.txt 
echo "checking for changes since date " $mydate >> $home_dir/extr_changes.txt
echo "" >> $home_dir/extr_changes.txt
#
printf "\n%-20s | %-10s | %-30s | %-30s | %-10s | %-20s | %-30s\n" REPO_NAME \ COMMIT_ID \ COMMIT_USER \ COMMIT_TIME \ PUB_STATE \ PUB_VER \ COMMIT_COMMENT  >> $home_dir/extr_changes.txt
#
...
#                echo "Repository is : " $repo >> $home_dir/extr_changes.txt
                IFS=";;"
...
#                                        echo $repo "|" $mycommit_id "|" $mycommit_user "|" $mycommit_time "|" $mycommit_comment "|" $mycommit_Publish "|" $my_publish_version >> $home_dir/extr_changes.txt
...
if  [ ! -f "$home_dir/extr_changes.txt" ]; then
  	echo "No changes found since last iso build: $mydate" >> $home_dir/extr_changes.txt
fi
